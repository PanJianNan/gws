...
<script>
    (function (a) {
        function c() {
            var b = a("#loading");
            if (b.size() == 0)b = a('<div id="loading" class="loading" style=\'z-index:3000\' />').appendTo(a("body")).hide();
            return b
        }
        ...
        a.Loading.show = function (b) {
            var d = c();
            b && d.html(b);
            a('<div style="height: 100%; width: 100%; position: fixed; left: 0pt; top: 0pt; z-index: 2999; opacity: 0.4;" class="jqmOverlay"></div>').appendTo(a("body"));
            d.show()
        };
...
<div style="width: 590px; padding: 10px; margin: 10px; border-width: 1px; border-color: rgb(102, 102, 102); border-style: solid none;">
    ...
    <table width="100%">
        <tbody>
        <tr style="background: none repeat scroll 0% 0% rgb(224, 234, 242);">
            <td width="60%">操作系统</td>
            <td>${osVersion }</td>
        </tr>
        <tr>
            <td width="60%">Java的运行环境</td>
            <td>${javaVersion }</td>
        </tr>
