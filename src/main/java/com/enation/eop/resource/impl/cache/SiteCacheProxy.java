...
public class SiteCacheProxy extends AbstractCacheProxy<EopSite> implements ISiteManager {
    ...
    public EopSite get(String domain) {
        ...
        if (site == null) {
            ...
            //todo 应用已安装过重启读取站点信息时可能会存在问题，如jdbc配置的是gws数据库，
            // 安装时改成mall数据库，发布的配置文件会变更，但是重启时又会使用源代码中的配置文件，
            // 结果就是启动失败，因为读取数据库信息时连的是gws，而数据却是初始化在mall数据库中的。
            // 所以建议第一次安装数据库时使用gws数据库，等这个问题解决之后再使用自定义数据库。
