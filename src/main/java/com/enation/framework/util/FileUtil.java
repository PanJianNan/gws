...
//			return allowTYpe.toString().indexOf(ex) >= 0;
    ...
    /**
     * 得到文件
     * PS:模仿 new File(), 但额外支持classpath路径下文件的读取
     *
     * @param resourceLocation 若是想获取classpath下的文件，请在在路径前添加 classpath: 如classpath:config/jdbc.properties<br/>
     *                         若无calsspath:前缀，则与new File()一致
     * @return
     */
    public static File getFile(String resourceLocation) {
        try {
            ...
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug("找不到文件：" + resourceLocation);
            }
            return new File("");//本返回null,但为了模拟new File()，所以返回一个不存在的File
        }
    }
