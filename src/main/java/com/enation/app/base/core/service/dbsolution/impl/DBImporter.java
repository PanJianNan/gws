...
public class DBImporter extends DBPorter {
    ...
    private List<Object> prepareValue(String values) {
        ...
        for (int i = 0; i < value.length; i++) {
            objects.add(solution.getFuncValue(solution.decodeValue(value[i].replaceAll("'", ""))));
        }
    ...
    /**
     * 执行action内容, 主要用于执行初始化SQL
     *
     * @param action
     * @return
     */
    private boolean doAction(Element action) {
        String command = action.elementText("command").toLowerCase();//SQL类型
        if ("create".equals(command)) {
            return this.doCreate(action);
        } else if ("insert".equals(command)) {
            return this.doInsert(action);
        }
        return true;
    }
    ...
    public boolean doImport(String xml) {
        ...
        try {
            ...
            } else if (xml.startsWith("classpath:")) {
                xml = FileUtil.readFile(xml.replaceAll("classpath:", ""));
                xmlDoc = DocumentHelper.parseText(xml);
            } else {
